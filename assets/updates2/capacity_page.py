...
countries_dict={'DEU':'Germany','AUT':'Austria','FRA':'France','BEL':'Belgium','NLD':'Netherlands','NO2':'Kristiansand',
                'DK1':'Aarhus','GBR':'United Kingdom','CHE':'Switzerland','CZE':'Czechia','DK2':'Copenhagen',
               'POL':'Poland','SE4':'Lund','ITA':'Italy','ESP':'Spain','NO5':'Bergen' }


countries_coordinates={'DEU': [51.0834196, 10.4234469], 'AUT': [47.59397, 14.12456], 'FRA': [46.603354, 1.8883335], 'BEL': [50.6402809, 4.6667145],
                       'NLD': [52.2288689, 5.3214503], 'NO2': [58.085627599999995, 7.93185802875815], 'DK1': [56.1496278, 10.2134046],
                       'GBR': [54.7023545, -3.2765753], 'CHE': [46.7985624, 8.2319736], 'CZE': [49.8167003, 15.4749544], 'DK2': [55.6867243, 12.5700724],
                       'POL': [52.215933, 19.134422], 'SE4': [55.7029296, 13.1929449], 'ITA': [42.6384261, 12.674297], 'ESP': [39.3260685, -4.8379791],
                       'NO5': [60.3943055, 5.3259192]}


def add_coordinates(df,countries_coordinates):
    origin_lon=[]
    origin_lat=[]
    dist_lon=[]
    dist_lat=[]
    origin_names=df['origin'].to_list()
    dist_names=df['distination'].to_list()

    for origin , dist in zip(origin_names, dist_names):
...
def add_lon_lat(df):
    ...
    for origin , dist in zip(origin_names, dist_names):
        ...
        if findGeocode(countries_dict[dist]) != None:

            dist_loc = findGeocode(countries_dict[dist])

            # coordinates returned from
            # function is stored into
            # two separate list
            dist_lat.append(dist_loc.latitude)
            dist_lon.append(dist_loc.longitude)
...
# min_cap (map_df['mean_cap'].min())
    ...
    for i in range(len(map_df)):
        cap = map_df['mean_cap'][i]

        scale=int(map_df['mean_cap'].max()/12 )
        line_size=int(map_df['mean_cap'][i]/scale)
        if line_size==0:
            line_size=1

        reversed_df=map_df[  (map_df['origin']==map_df['distination'][i])  & (map_df['distination']==map_df['origin'][i]) ]
        if reversed_df.empty:
    ...
    for ind in nodes_df.index:
        ...
        for i , row in hover_df.iterrows():
            hov_string='{} : {}'.format(row['countries'],row['mean_cap'])
